...
class Argparse:
    ...
    def convert_fn(self, name, data, p=None):
        ...
        log.debug("fn %s, args=%s", name, [x.get("name") for x in data.get("args", [])])
    ...
    def fn_arg(self, fdata, parsed):
        ...
        for ag in fdata.get("args", []):
            ...
            if not hasattr(parsed, name):
                ...
                continue
            if (
                ag.get("kind") in (Parameter.POSITIONAL_ONLY, Parameter.VAR_POSITIONAL)
                or ag.get("kind") == Parameter.POSITIONAL_OR_KEYWORD
                and self.positional
            ):
    ...
    def split_arg(self, data, parsed):
        ...
        farg_kw, farg_pos = self.fn_arg(data.get(parsed.__FUNCNAME__, {}), parsed)
